...
export const batchAllocationApi = {
  createBatchId: () => request.post("/batch-allocation/batch-id"),
  downloadTemplate: () => "/api/batch-allocation/template",
  downloadTestData: () => "/api/batch-allocation/test-data",
  importExcel: (formData, batchId) =>
    request.post("/batch-allocation/import", formData, {
      params: { batchId },
      timeout: 60000,
    }),
  allocate: (batchId) =>
    request.post("/batch-allocation/allocate", null, { params: { batchId } }),
  getPreview: (batchId) =>
    request.get("/batch-allocation/preview", { params: { batchId } }),
  confirm: (batchId) =>
    request.post("/batch-allocation/confirm", null, { params: { batchId } }),
  swap: (batchId, data) =>
    request.post("/batch-allocation/swap", data, { params: { batchId } }),
};
